...
<?php
	...
	$conndb = mysql_select_db("$DBName") or die("geen database beschikbaar met naam $DBName");
		$queryklanten = mysql_query ("SELECT klantnummer FROM offertes WHERE id='" .$_GET["id"] ."'");
...
<span style="font-size:4.0pt;margin-left:5pt">&nbsp;</span><br>
<span style="font-size:12.0pt;font-family:Arial;margin-left:5pt"><b>OFFERTE</b></span>
<span style="font-size:11.0pt;font-family:Arial;margin-left:43pt">Klantnr.: <?php  $leadingzerosid = sprintf("%04s",$klantid);   echo "$leadingzerosid"; ?></span>
<span style="font-size:11.0pt;font-family:Arial;margin-left:43pt">Referentienr.: <?php $offerteid = $_GET["id"];   $leadingzerosfactid = sprintf("%05s",$offerteid);   echo "$leadingzerosfactid"; ?></span>
<span style="font-size:12.0pt;font-family:Arial;margin-left:43pt"><b>

<?php

include ('include/datums.inc.php');

		$result = mysql_query("SELECT datum FROM offertes WHERE id='" .$_GET["id"] ."'");
		while ($rij = mysql_fetch_array($result)) {
...
<?php
		$temp = $_GET["id"] - 1;
		$result = mysql_query("SELECT id, omschrijving, aantal, stukprijs, bedrag FROM offerteomschrijving WHERE offertenummer=$temp ORDER BY id");
		while ($rij = mysql_fetch_array($result)) {
...
<tr><td>
<?php
		$result = mysql_query("SELECT totaalexcl, btwbedrag, totaalincl FROM offertes WHERE id='" .$_GET["id"] ."'");

...
my_window.document.write('<center><br><a href="betaling.php?id=<?php echo "$factid"; ?>&bet=cash">Cash</a>, <a href="betaling.php?id=<?php echo "$factid"; ?>&bet=niet">Niet betaald</a>, <a href="betaling.php?id=<?php echo "$factid"; ?>&bet=over">Overschrijving</a>, <a href="betaling.php?id=<?php echo "$factid"; ?>&bet=cheque">Cheque</a></center>');
}
</SCRIPT>
...
<?php
	$conn = mysql_connect("$DBHost", "$DBUser", "$DBPass") or die("geen verbinding");
	$conndb = mysql_select_db("$DBName") or die("geen database beschikbaar met naam $DBName");
		$queryklanten = mysql_query ("SELECT klantnummer FROM facturen WHERE id='" .$_GET["id"] ."'");
		$klantid = mysql_result($queryklanten, 0, 0);
		$result = mysql_query("SELECT klantnaam, klantstraat, klantplaats, klanttelefoon, klantbtw FROM KLANTEN WHERE id=$klantid");
...
<?php
        ...
        $result = mysql_query("SELECT totaalexcl FROM facturen WHERE id='" .$_GET["id"] ."'");
		$rij = mysql_fetch_array($result);
        if (($rij[totaalexcl]) < 0) { echo "<span style=\"font-size:12.0pt;font-family:Arial;margin-left:5pt\"><b>CREDITNOTA</b></span><span style=\"font-size:11.0pt;font-family:Arial;margin-left:26pt\">"; }
                               else { echo "<span style=\"font-size:12.0pt;font-family:Arial;margin-left:5pt\"><b>FACTUUR</b></span><span style=\"font-size:11.0pt;font-family:Arial;margin-left:46pt\">"; }

?>

                                                                  Klantnr.: <?php  $leadingzerosid = sprintf("%04s",$klantid);   echo "$leadingzerosid"; ?></span>
<span style="font-size:11.0pt;font-family:Arial;margin-left:46pt">Factuurnr.: <?php  $leadingzerosfactid = sprintf("%05s",$factid);   echo "$leadingzerosfactid"; ?></span>
...
include ('include/datums.inc.php');
		...
		$result = mysql_query("SELECT datum FROM facturen WHERE id='" .$_GET["id"] ."'");
		while ($rij = mysql_fetch_array($result)) {
...
<tr><td>
<?php
		$result = mysql_query("SELECT totaalexcl, btwbedrag, totaalincl FROM facturen WHERE id='" .$_GET["id"] ."'");
		$rij = mysql_fetch_array($result);

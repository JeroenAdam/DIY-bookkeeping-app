...
<?php
	...
	$conndb = mysql_select_db("$DBName") or die("geen database beschikbaar met naam $DBName");
		...
		$laatsteofferte = mysql_result($queryrecords, 0, 0);
		$queryklanten = mysql_query ("SELECT klantnummer FROM offertes WHERE id=$laatsteofferte");
...
<span style="font-size:11.0pt;font-family:Arial;margin-left:43pt">Klantnr.: <?php  $leadingzerosid = sprintf("%04s",$laatsteklantid);   echo "$leadingzerosid"; ?></span>
<span style="font-size:11.0pt;font-family:Arial;margin-left:43pt">Referentienr.: <?php $leadingzerosofferteid = sprintf("%05s",$laatsteofferte);   echo "$leadingzerosofferteid"; ?></span>
<span style="font-size:12.0pt;font-family:Arial;margin-left:43pt"><b>
<?php

include ('include/datums.inc.php');

		$result = mysql_query("SELECT datum FROM offertes WHERE id=$laatsteofferte");
		while ($rij = mysql_fetch_array($result)) {
...
<tr><td>
<?php
		$result = mysql_query("SELECT totaalexcl, btwbedrag, totaalincl FROM offertes WHERE id=$laatsteofferte");

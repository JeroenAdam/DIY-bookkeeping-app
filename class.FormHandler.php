...
class FormHandler {
    ...
    function FormHandler($dbName = false, $dbTable = false, $dbPrKey = false) {
        ...
        $this->fckFile        = "http://".$_SERVER["HTTP_HOST"]."/includes/FCKeditor/fckeditor.html";  // path to the FCK file!!!!!
    ...
    function DateField ($title, $name, $interval = "90:0", $callback = false, $format = 'd-m-y', $extra = '') {
        ...
        $format = str_replace('Y', $jaar, $format);
        $format = str_replace('y', (strlen($jaar) == 2 ? $jaar : substr($jaar, 2, 2)), $format);
		$error = $this->callUserFunction($callback, date($format, mktime(0,0,0,$maand,$dag,date('Y'))));
        if($this->posted)
    ...
    function Editor($title, $name, $config = array(), $callback = false, $pxWidth = 620, $pxHeight = 400) {
